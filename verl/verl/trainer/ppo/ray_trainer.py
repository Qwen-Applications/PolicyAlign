...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.teacher_mode = config.trainer.get("teacher_mode", "frozen")
        self.teacher_ema_decay = float(config.trainer.get("teacher_ema_decay", 0.999))
        assert self.teacher_mode in ["frozen", "ema", "student"], f"Unknown teacher_mode: {self.teacher_mode}"
        ...
        if config.algorithm.use_kl_in_reward:
            if self.teacher_mode != "frozen":
                raise NotImplementedError(
                    "teacher_mode=student/ema currently supports actor.use_kl_loss path only; "
                    "algorithm.use_kl_in_reward still expects ref_log_prob from standalone RefPolicy."
                )
            self.kl_ctrl_in_reward = core_algos.get_kl_controller(config.algorithm.kl_ctrl)
    ...
    def _validate_system_prompt(self):
        ...
        HELD_OUT_SIZE = max(HELD_OUT_SIZE, len(self.val_dataloader))
        print("HELD_OUT_SIZE: ", HELD_OUT_SIZE)
        HELD_OUT_ROLLOUT = self.config.trainer.get("held_out_rollout", 1)
        ...
        if eval_prepend_experience:
            ...
            print(f"[Validation] eval_prepend_experience=True, loaded experience from {exp_path}")

        print(f"[Validation] eval_wo_experience=True, eval_prepend_experience={eval_prepend_experience}, evaluating on HELD_OUT only...")
        heldout_gen_list = []
        heldout_meta_list = []
        ...
        for heldout_data in heldout_data_list:
            ...
            if eval_prepend_experience:
                msgs = deepcopy(hb_gen.non_tensor_batch['raw_prompt'][0])
                if isinstance(msgs, np.ndarray):
                    msgs = msgs.tolist()
                if EXPERIENCE:
                    msgs.insert(0, {"role": "system", "content": EXPERIENCE})
                tokenized = self.val_dataset.re_tokenize(msgs)
                
                hb_gen.batch["input_ids"] = tokenized["input_ids"].unsqueeze(0)
                hb_gen.batch["attention_mask"] = tokenized["attention_mask"].unsqueeze(0)
                hb_gen.batch["position_ids"] = tokenized["position_ids"].unsqueeze(0)
        ...
        metrics = {}
        if self.config.trainer.system_prompt_type == 'medmcqa':
            accs = get_medmcqa_acc(eval_batch, self.tokenizer, self.config.trainer.system_prompt_version)
            acc = sum(accs) / len(accs)
            metrics = {"held_out_acc": acc}
        elif self.config.trainer.system_prompt_type == 'safety':
            accs, by_source_stats = get_safety_acc(eval_batch=eval_batch, tokenizer=self.tokenizer, return_by_source=True)
            acc = sum(accs) / len(accs)
            for src, stats in by_source_stats.items():
                print(f"[Validation] data_source={src}, acc={stats['acc']:.4f}, n={stats['n']}")
                metrics[f"test_acc_{src}"] = stats["acc"]

        sample_scores.append(acc)
        print(f"[Validation] Batch Acc: {acc}")
        ...
        if val_data_dir:
            ...
            with open(os.path.join(val_data_dir, f"samples_step_{step}.json"), "w") as f:
                json.dump(saved_samples, f, ensure_ascii=False, indent=2)

            print(
                f"[Validation eval_wo_experience] "
                f"Saved {len(saved_samples)} random samples to "
                f"{val_data_dir}/random_samples_step_{step}.json"
            )
    ...
    def _validate(self):
        ...
        if self.config.trainer.setting == "textgame":
            return self._validate_textgame()

        if self.config.trainer.train_system_prompt:
            return self._validate_system_prompt()
        ...
        if eval_wo_experience:
            ...
            for heldout_data in heldout_data_list:
                ...
                if eval_prepend_experience:
                    ...
                    if EXPERIENCE:
                        updated_content = EXPERIENCE_SOLVE_PROMPT_TEMPLATE.format(experience=EXPERIENCE, prompt=content)
                    else:
            ...
            acc = (reward_tensor.sum(-1) == 1).float().mean().item()
            sample_scores.append(acc)
            print(f"[Validation eval_wo_experience] Heldout Batch Acc: {acc}")

            # Randomly select and save 5 samples
            num_samples_to_save = min(20, len(responses))
            sample_indices = random.sample(range(len(responses)), num_samples_to_save)
            saved_samples = []
            for idx in sample_indices:
                ...
                saved_samples.append({
                    "prompt": prompt_text,
                    "response": response_text,
                    "reward": reward_tensor[idx].sum(-1).item()
                })
        ...
        consumed_samples = 0
        limit = self.config.trainer.get("val_samples_limit", len(self.val_dataloader))
        while consumed_samples < limit:
            try:
                test_data = next(val_iterator)
            except StopIteration:
                break
            consumed_samples += 1
            ...
            # === Step 1: Evaluate on heldout Batch ===
            heldout_gen_list = []
            heldout_meta_list = []
            ...
            for heldout_data in heldout_data_list:
                ...
                # Update prompt with current EXPERIENCE
                msgs = deepcopy(hb_gen.non_tensor_batch['raw_prompt'][0])
                content = msgs[-1]['content']
                if EXPERIENCE:
                    updated_content = EXPERIENCE_SOLVE_PROMPT_TEMPLATE.format(experience=EXPERIENCE, prompt=content)
            ...
            # Calculate average response token length
            responses = heldout_output.batch["responses"]
            # Count non-padding tokens in each response
            response_lengths = (responses != self.tokenizer.pad_token_id).sum(dim=-1).float()
            avg_response_length = response_lengths.mean().item()
            sample_response_lengths.append(avg_response_length)
            print(f"[Validation Step {consumed_samples}] Avg Response Token Length: {avg_response_length:.2f}")
            ...
            acc = (reward_tensor.sum(-1) == 1).float().mean().item()
            sample_scores.append(acc)
            print(f"[Validation Step {consumed_samples}] Heldout Batch Acc: {acc}")
            print(f"History acc: {sample_scores}")
            
            # === Step 2: Generate new experience using test_batch ===
            ...
            # Update prompt with current EXPERIENCE
            msgs = deepcopy(gen_batch.non_tensor_batch['raw_prompt'][0])
            content = msgs[-1]['content']
            
            # NOTE: Currently we always use empty exp for exp_gen_exp
            updated_content = content

            msgs[-1]['content'] = updated_content
            tokenized = self.val_dataset.re_tokenize(msgs)
            
            gen_batch.batch["input_ids"] = tokenized["input_ids"].unsqueeze(0)
            gen_batch.batch["attention_mask"] = tokenized["attention_mask"].unsqueeze(0)
            gen_batch.batch["position_ids"] = tokenized["position_ids"].unsqueeze(0)
            ...
            gen_batch_padded.meta_info["n"] = 1 # Generate 1 sequence per input
            ...
            output_text = self.tokenizer.decode(gen_output.batch["responses"][0], skip_special_tokens=True)

            # === Step 3: Generate new experience ===
            latest_experience = f"Input: {updated_content}\nOutput: {output_text}"
            ...
            exp_learner_tokenized = self.val_dataset.re_tokenize(exp_learner_msgs)
            
            exp_learner_batch = DataProto.from_single_dict({
                "input_ids": exp_learner_tokenized["input_ids"].unsqueeze(0),
                "attention_mask": exp_learner_tokenized["attention_mask"].unsqueeze(0),
                "position_ids": exp_learner_tokenized["position_ids"].unsqueeze(0),
            })
            ...
            exp_learner_batch_padded, exp_pad_size = pad_dataproto_to_divisor(exp_learner_batch, self.exp_learner_wg.world_size)
            ...
            exp_text = self.tokenizer.decode(exp_learner_output.batch["responses"][0], skip_special_tokens=True)
            if "</think>" in exp_text:
                exp_text = exp_text.split("</think>")[-1]
            
            # Only keep lines starting with experience marker, handle multiple per line
            if self.config.trainer.prompt_version == 'v2':
                ...
                for line in exp_lines:
                    parts = line.split(marker)[1:]  # skip first empty element
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.ActorRollout)
            actor_worker_role = "actor_rollout_ref" if self.teacher_mode == "ema" else "actor_rollout"
            actor_rollout_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.ActorRollout],
                config=self.config.actor_rollout_ref,
                role=actor_worker_role,
            )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in dataloader_iter:
                ...
                if self.config.trainer.stage == "consolidate":
                    ...
                    metrics = {}
                    timing_raw = {}
                    # Handle textgame mode differently
                    if self.is_textgame:
                    ...
                    with marked_timer("step", timing_raw):
                        with marked_timer("gen", timing_raw, color="red"):
                            if not self.is_textgame:
                                ...
                                for i in range(len(gen_batch_with_exp)):
                                    ...
                                    if self.global_steps == 1 and i == 0:
                                        print('msgs example after: ', msgs)
                            ...
                            if self.is_textgame:
                                if self.config.trainer.on_policy_merge or self.config.trainer.generate_off_policy:
                                    if self.config.trainer.on_policy_merge:
                                        ...
                                        chosen_experiences = [rng.choice(EXPERIENCES) for _ in range(env_num)]
                                        gen_output = self.actor_rollout_wg.generate_sequences_textgame(
                                            env_config=self.textgame_env_config,
                                            env_num=env_num,
                                            tokenizer=self.tokenizer,
                                            experiences=chosen_experiences,
                                            num_steps=num_steps,
                                            seeds=seeds
                                        )
                                    ...
                                    # the reward scalar is not used for RL. just for logging to monitor training process
                                    gen_output_rewards = [reward_dict[0] for reward_dict in gen_output["reward_list"]]
                                    ...
                                    for env_idx, traj in env_trajectories.items():
                                        ...
                                        env_exp = chosen_experiences[env_idx] if not self.config.trainer.on_policy_merge else ""
                                        for step_item in history:
                                            ...
                                            exp_per_sample.append(env_exp)

                                    gen_batch_output = self._build_dataproto(prompt_token_ids_list, response_ids_list)
                                    gen_batch_output.non_tensor_batch['raw_prompt'] = np.array(raw_prompt_list, dtype=object)
                                    gen_batch_output.non_tensor_batch['exp_per_sample'] = np.array(exp_per_sample, dtype=object)

                                    # gen_batch_output should be multiple of self.actor_rollout_wg.world_size, we drop some items here
                                    gen_batch_output = gen_batch_output[:len(gen_batch_output) - len(gen_batch_output) % self.actor_rollout_wg.world_size]

                                    if self.config.trainer.on_policy_merge:
                                        batch = gen_batch_output.select(deepcopy=True)
                                        batch_with_exp = None
                                    elif self.config.trainer.generate_off_policy:
                                        batch_with_exp = gen_batch_output.select(deepcopy=True)
                                        batch = None
                                        
                                        exp_name = self.config.trainer.experiment_name
                                        train_step = self.global_steps
                                        save_dir = self.config.trainer.off_policy_save_dir
                                        os.makedirs(save_dir, exist_ok=True)
                                        save_path = os.path.join(save_dir, f"gen_batch_output_step_{train_step}.pkl")
                                        with open(save_path, "wb") as f:
                                            pickle.dump(gen_batch_output, f)
                                        print(f"[Off-Policy Save] Saved gen_batch_output to {save_path}")

                                        if not (self.config.actor_rollout_ref.actor.kl_loss_type == "full" and self.config.actor_rollout_ref.actor.kl_topk > 0):
                                            is_last_step = self.global_steps >= self.total_training_steps
                                            self.global_steps += 1
                                            if is_last_step:
                                                progress_bar.close()
                                    ...
                                    exp_name = self.config.trainer.experiment_name
                                    train_step = self.global_steps
                                    load_dir = self.config.trainer.off_policy_save_dir
                                    load_path = os.path.join(load_dir, f"gen_batch_output_step_{train_step}.pkl")
                                    assert os.path.exists(load_path)
                                    with open(load_path, "rb") as f:
                                        gen_batch_output = pickle.load(f)
                                    print(f"[Off-Policy Load] Loaded gen_batch_output from {load_path}")
                                    gen_batch_output = gen_batch_output[:len(gen_batch_output) - len(gen_batch_output) % self.actor_rollout_wg.world_size]
                                    batch_with_exp = gen_batch_output.select(deepcopy=True)
                                    batch = None
                                    # Restore exp_per_sample saved during generate_off_policy
                                    if 'exp_per_sample' in gen_batch_output.non_tensor_batch:
                                        exp_per_sample = list(gen_batch_output.non_tensor_batch['exp_per_sample'])
                                    else:
                                        print("[Warning] exp_per_sample not found in loaded gen_batch_output, falling back to empty experiences")
                                        exp_per_sample = [""] * len(gen_batch_output)
                                
                                if batch_with_exp is None:
                                    ...
                                    gen_batch = batch_with_exp.select(deepcopy=True)
                                    
                                    updated_gen_inputs = []
                                    for i in range(len(gen_batch)):
                                        msgs = deepcopy(gen_batch.non_tensor_batch['raw_prompt'][i])
                                        content = msgs[-1]['content']
                                        exp_to_use = exp_per_sample[i]
                                        if exp_to_use:
                                            remove_str = f"You are an agent playing a game on a grid, acting as a reasoning engine.\n\nYour decisions are based on the experience you have learned about the game's rules or strategies. This experience is only a guess of how the game works, and the rules and strategies may be incomplete or incorrect.\n\nGiven experience for rules or strategies you have learned:\n{exp_to_use}\n\nCurrent situation:\n"
                                            add_str = "You are an agent playing a game on a grid, acting as a reasoning engine.\n\nCurrent situation:\n"
                                            updated_content = add_str + content[len(remove_str):]
                                        else:
                                            updated_content = content
                                        msgs[-1]['content'] = updated_content
                                    ...
                                    gen_batch.batch["input_ids"] = torch.stack([inp["input_ids"] for inp in updated_gen_inputs])
                                    gen_batch.batch["attention_mask"] = torch.stack([inp["attention_mask"] for inp in updated_gen_inputs])
                                    gen_batch.batch["position_ids"] = torch.stack([inp["position_ids"] for inp in updated_gen_inputs])
                                    gen_batch.non_tensor_batch.pop("raw_prompt_ids", None)
                                    gen_batch.non_tensor_batch.pop("raw_prompt", None)

                                    batch = gen_batch.select(deepcopy=True)
                                ...
                                if self.config.trainer.on_policy_merge or self.config.trainer.generate_off_policy:
                                    if not self.async_rollout_mode:
                                        if self.config.trainer.on_policy_merge:
                                            ...
                                            gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch_with_exp)
                                    else:
                                        self.async_rollout_manager.wake_up()
                                        if self.config.trainer.on_policy_merge:
                                            gen_batch_output = self.async_rollout_manager.generate_sequences(gen_batch)
                                        else:
                                            gen_batch_output = self.async_rollout_manager.generate_sequences(gen_batch_with_exp)
                                        self.async_rollout_manager.sleep()
                                    
                                    if self.config.trainer.generate_off_policy:
                                        exp_name = self.config.trainer.experiment_name
                                        train_step = self.global_steps
                                        save_dir = self.config.trainer.off_policy_save_dir
                                        os.makedirs(save_dir, exist_ok=True)
                                    ...
                                    if not os.path.exists(load_path):
                                        if self.config.actor_rollout_ref.actor.kl_loss_type == "seqkd":
                                            teacher_response = batch_with_exp.batch["teacher_response"]
                                            if teacher_response is None:
                                                raise ValueError("[SeqKD Mode] teacher_response not found in batch_with_exp. Make sure your dataset includes teacher_response field.")
                                            
                                            prompts = gen_batch_with_exp.batch["input_ids"]
                                            responses = teacher_response
                                            batch_dict = {
                                                "prompts": prompts,           # (bs, prompt_length)
                                                "responses": responses,       # (bs, response_length)
                                            }
                                            
                                            batch_size = prompts.size(0)
                                            batch_in_gen = TensorDict(batch_dict, batch_size=batch_size)
                                            non_tensor_batch = gen_batch_with_exp.non_tensor_batch if hasattr(gen_batch_with_exp, 'non_tensor_batch') else {}
                                            gen_batch_output = DataProto(batch=batch_in_gen, non_tensor_batch=non_tensor_batch, meta_info={"timing": {}})
                                            assert self.config.actor_rollout_ref.rollout.n == 1
                                            
                                            print(f"[SeqKD Mode] Manually created gen_batch_output using teacher_response (batch_size={batch_size}, prompt_len={prompts.size(1)}, response_len={responses.size(1)}) at step {self.global_steps}")
                                        else:
                                            raise FileNotFoundError(f"[Off-Policy Load] Could not find saved gen_batch_output at {load_path}")
                                    else:
                                        with open(load_path, "rb") as f:
                                            gen_batch_output = pickle.load(f)
                                        print(f"[Off-Policy Load] Loaded gen_batch_output from {load_path}")
                                        assert (gen_batch_output.batch["prompts"][0::self.config.actor_rollout_ref.rollout.n] == gen_batch_with_exp.batch["input_ids"]).all()
                        ...
                        if self.config.trainer.on_policy_merge:
                            ...
                            batch_with_exp.non_tensor_batch["uid"] = np.array([str(uuid.uuid4()) for _ in range(len(batch_with_exp.batch))], dtype=object)
                            batch_with_exp = batch_with_exp.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                            batch_with_exp = batch_with_exp.union(gen_batch_output)
                            if "attention_mask" in batch_with_exp.batch:
                                batch_with_exp.batch["response_mask"] = compute_response_mask(batch_with_exp)
                            ...
                            prompts_wo_exp = gen_batch.batch["input_ids"]  # (bs, prompt_length)
                            prompt_attention_mask = gen_batch.batch["attention_mask"]
                            prompt_position_ids = gen_batch.batch["position_ids"]
                            responses = gen_batch_output.batch["responses"]  # (bs, response_length)
                            
                            seq_wo_exp = torch.cat([prompts_wo_exp, responses], dim=-1)
                            
                            eos_token_id = batch.meta_info.get("eos_token_id", self.tokenizer.eos_token_id)
                            response_attention_mask = get_response_mask(response_id=responses, eos_token=eos_token_id, dtype=prompt_attention_mask.dtype)
                            attention_mask_wo_exp = torch.cat([prompt_attention_mask, response_attention_mask], dim=-1)
                            
                            response_length = responses.size(1)
                            delta_position_id = torch.arange(1, response_length + 1, device=prompt_position_ids.device)
                            delta_position_id = delta_position_id.unsqueeze(0).expand(prompts_wo_exp.size(0), -1)
                            if prompt_position_ids.dim() == 3:  # qwen2vl mrope
                                delta_position_id = delta_position_id.view(prompts_wo_exp.size(0), 1, -1).expand(prompts_wo_exp.size(0), 3, -1)
                            response_position_ids = prompt_position_ids[..., -1:] + delta_position_id
                            position_ids_wo_exp = torch.cat([prompt_position_ids, response_position_ids], dim=-1)
                            
                            batch.batch["prompts"] = prompts_wo_exp
                            batch.batch["responses"] = responses
                            batch.batch["input_ids"] = seq_wo_exp
                            batch.batch["attention_mask"] = attention_mask_wo_exp
                            batch.batch["position_ids"] = position_ids_wo_exp
                            batch.batch["response_mask"] = compute_response_mask(batch)
                        ...
                        if not self.config.trainer.skip_reward:
                            with marked_timer("reward", timing_raw, color="yellow"):
                                if self.is_textgame:
                                ...
                                elif self.config.trainer.train_system_prompt:
                                    if self.config.trainer.system_prompt_type == 'medmcqa':
                                        accs = get_medmcqa_acc(batch, self.tokenizer, self.config.trainer.system_prompt_version)
                                    elif self.config.trainer.system_prompt_type == 'safety':
                                        accs = get_safety_acc(batch, self.tokenizer)
                                    acc = sum(accs) / len(accs)
                                    metrics.update({
                                        "actor/curr_acc": acc
                                    })
                                    ...
                                    if self.use_rm:
                                        if self.config.reward_model.strategy == 'vllm':
                                            arena_data = self.rm_wg.prepare_arena_matches(batch)
                                            gen_arena_output = self.rm_wg.generate_arena_responses(arena_data)
                                            reward_tensor = self.rm_wg.postprocess_arena_results_to_rewards(batch, arena_data, gen_arena_output)
                                        else:
                                    ...
                                    if self.use_rm:
                                        if self.config.reward_model.strategy == 'vllm':
                                            arena_data = self.rm_wg.prepare_arena_matches(batch_prev)
                                            gen_arena_output = self.rm_wg.generate_arena_responses(arena_data)
                                            reward_tensor_prev = self.rm_wg.postprocess_arena_results_to_rewards(batch_prev, arena_data, gen_arena_output)
                                        else:
                                            reward_tensor_prev = self.rm_wg.compute_rm_score(batch_prev)
                                        
                                        batch_prev = batch_prev.union(reward_tensor_prev)
                                    
                                    reward_tensor = reward_tensor.sum(dim=-1)

                                    metrics.update({
                                        "actor/curr_acc": reward_tensor.mean().item() / 2 + 0.5
                                    })
                            ...
                            if self.config.trainer.on_policy_merge or self.config.trainer.generate_off_policy:
                                with marked_timer("compute_topk_indices", timing_raw, color="purple"):
                                    if self.config.trainer.on_policy_merge:
                                        ...
                                        log_prob_proto = self.actor_rollout_wg.compute_log_prob(batch_with_exp)
                                    ...
                                    if self.config.trainer.generate_off_policy:
                                        ...
                                        save_dir = self.config.trainer.off_policy_save_dir
                                        os.makedirs(save_dir, exist_ok=True)
                                        save_path = os.path.join(save_dir, f"kl_topk_indices_step_{train_step}.pkl")
                                        with open(save_path, "wb") as f:
                                            pickle.dump(actor_topk_indices, f)
                                        print(f"[Off-Policy Save] Saved kl_topk_indices to {save_path}")
                                        is_last_step = self.global_steps >= self.total_training_steps
                                        self.global_steps += 1
                                        if is_last_step:
                                            progress_bar.close()
                                ...
                                # Load kl_topk_indices from saved from generate_off_policy
                                exp_name = self.config.trainer.experiment_name
                                train_step = self.global_steps
                                load_dir = self.config.trainer.off_policy_save_dir
                                load_path = os.path.join(load_dir, f"kl_topk_indices_step_{train_step}.pkl")
                                if not os.path.exists(load_path):
                                    raise FileNotFoundError(f"[Off-Policy Load] Could not find saved kl_topk_indices at {load_path}")
                                with open(load_path, "rb") as f:
                                    saved_topk_indices = pickle.load(f)
                                saved_topk_indices = saved_topk_indices[:len(batch)]
                                print(f"[Off-Policy Load] Loaded kl_topk_indices from {load_path}")
                                
                                with marked_timer("compute_topk_indices", timing_raw, color="purple"):
                                    if self.config.actor_rollout_ref.actor.kl_merge_indice:
                                        batch.batch["first_kl_topk_indices"] = saved_topk_indices
                                        log_prob_proto = self.actor_rollout_wg.compute_log_prob(batch)
                                        log_probs = log_prob_proto.batch["old_log_probs"]
                                        merged_indices = log_probs.long()

                                        batch_with_exp.batch["kl_topk_indices"] = merged_indices

                                        del log_probs
                                        del log_prob_proto
                                    
                                    else:
                                        batch_with_exp.batch["kl_topk_indices"] = saved_topk_indices
                        ...
                        if self.config.actor_rollout_ref.actor.kl_loss_type != "seqkd":
                            with marked_timer("exp_log_prob", timing_raw, color="olive"):
                                ...
                                if self.teacher_mode == "frozen":
                                    assert self.use_reference_policy
                                    exp_log_prob = self.ref_policy_wg.compute_ref_log_prob(batch_with_exp)
                                    teacher_key = "ref_log_prob"
                                elif self.teacher_mode == "ema":
                                    exp_log_prob = self.actor_rollout_wg.compute_ref_log_prob(batch_with_exp)
                                    teacher_key = "ref_log_prob"
                                else:  # student
                                    exp_log_prob = self.actor_rollout_wg.compute_log_prob(batch_with_exp)
                                    teacher_key = "old_log_probs"
                                ...
                                exp_log_prob.batch["exp_log_probs"] = exp_log_prob.batch[teacher_key]
                                exp_log_prob.batch.pop(teacher_key)
                        ...
                        if self.config.trainer.critic_warmup <= self.global_steps:
                            ...
                            if self.teacher_mode == "ema":
                                self.actor_rollout_wg.ema_update_internal_ref(decay=self.teacher_ema_decay)
