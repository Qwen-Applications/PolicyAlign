...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    @torch.no_grad()
    def ema_update_internal_ref(self, decay: float = 0.999):
        """
        EMA update for internal teacher.
        This is only used when the worker is created with role='actor_rollout_ref'
        and trainer.teacher_mode == 'ema'.
        """
        assert self._is_actor and self._is_ref, (
            "ema_update_internal_ref requires an actor_rollout_ref worker "
            "that contains both actor and ref."
        )
        ...
        ref_module = self.ref_policy.actor_module

        actor_named_params = dict(actor_module.named_parameters())
        ref_named_params = dict(ref_module.named_parameters())
        if actor_named_params.keys() != ref_named_params.keys():
            raise RuntimeError("Actor/ref parameter names do not match for EMA update.")

        for name, ref_param in ref_named_params.items():
            actor_param = actor_named_params[name]
            src = actor_param.data.to(device=ref_param.data.device, dtype=ref_param.data.dtype)
            if ref_param.data.is_floating_point():
                ref_param.data.mul_(decay).add_(src, alpha=1.0 - decay)
            else:
                ref_param.data.copy_(src)

        actor_named_buffers = dict(actor_module.named_buffers())
        ref_named_buffers = dict(ref_module.named_buffers())
        if actor_named_buffers.keys() != ref_named_buffers.keys():
            raise RuntimeError("Actor/ref buffer names do not match for EMA update.")

        for name, ref_buf in ref_named_buffers.items():
            actor_buf = actor_named_buffers[name]
            src = actor_buf.data.to(device=ref_buf.data.device, dtype=ref_buf.data.dtype)
            ref_buf.data.copy_(src)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during ema_update_internal_ref", logger=logger)
